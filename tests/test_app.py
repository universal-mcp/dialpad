...
@pytest.fixture
def app_instance():
    """Provides a DialpadApp instance for tests."""
    mock_integration = MagicMock()
    mock_integration.get_credentials.return_value = {"access_token": "dummy_access_token"}

    return DialpadApp(integration=mock_integration)

def test_universal_mcp_dialpad_app_initialization(app_instance):
    """
    Test that the DialpadApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "dialpad", "DialpadApp instance has unexpected name."


def test_universal_mcp_dialpad_tool_docstrings_format(app_instance):
    """
    Test that each tool method in DialpadApp has a well-formatted docstring,
...
def test_universal_mcp_dialpad_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in DialpadApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
